...
public class PeerProcess extends Thread implements Serializable {
	...
	public HashSet<serverConnect> es = new HashSet<serverConnect> ();
	static Hashtable<Integer, clientConnect> esclientmap = new Hashtable<Integer, clientConnect>();
	...
	public void getConfiguration()
	{
		...
		try {
			//String FileName = System.getProperty("user.dir")+"/src/FileShare/PeerInfo.cfg";
			String FileName = System.getProperty("user.dir")+"/FileShare/PeerInfo.cfg";
	...
	public static void main (String []args) throws IOException {
		...
		//while (true) {
			...
			//servCon = new serverConnect(myPeerId, pTemp, connectionSocket);
			
			//Thread t = new Thread(servCon);
			
			//servCon = new serverConnect(myPeerId, pTemp, connectionSocket);
			//servCon = new serverConnect(myPeerId, pTemp, peerPort);
	...
	void connectionSetup () throws UnknownHostException, IOException {
		...
		//clientConnect []ClientThreads = new clientConnect[toAccessVector.peerInfoVector.size()];
		//clientConnect []ClientThreads = new clientConnect[pTemp.peerInfoVector.size()];
		//clientConnect ClientThreads;
		//pTemp.log = new P2PLog[pTemp.peerInfoVector.size()+1];
		this.log = new P2PLog(this.iPeerID);
		...
		//	pInfo = (RemotePeerInfo) toAccessVector.peerInfoVector.elementAt(i);
			...
			if (clientPeerId == this.iPeerID) {
				...
				try {
					...
					clientConnect ec = new clientConnect(clientSock, this.iPeerID, Integer.parseInt(pInfo.peerId), this);
					...
					//ClientThreads = new clientConnect(myPeerId, pInfo.peerId, pInfo.peerAddress, pInfo.peerPort, pTemp.myBitFields, pTemp);
